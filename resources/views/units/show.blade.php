<x-layouts.app>
    @php
        $sortLink = fn ($col) => route('units.show', $unit->id).'?'.http_build_query(array_merge(
    ...
    <div class="page-header">
        <div>
            <div class="page-header__eyebrow">{{ str_replace('_', ' ', $unit->type) }} @if ($unit->parent) — {{ $unit->parent->name }} @endif</div>
            <h1><span class="code-chip">{{ $unit->code }}</span> {{ $unit->name }}</h1>
            <p>
                Kepala Unit: {{ $unit->head?->name ?? '-' }}
                @if ($unit->children->count() > 0)
                    &nbsp;·&nbsp; {{ $unit->children->count() }} sub-unit
                @endif
            </p>
        </div>
        <div style="display:flex; gap:8px;">
            <a href="{{ route('units.edit', $unit->id) }}" class="btn btn-outline">✏ Edit Unit</a>
    ...
    <div class="stat-grid" style="grid-template-columns: repeat(3, 1fr);">
        <div class="stat-card">
            ...
            <div class="stat-card__label">Total Aset Unit Ini</div>
        </div>
        <div class="stat-card">
    ...
    <div class="card">
        ...
        <form method="GET" action="{{ route('units.show', $unit->id) }}" class="filters">
            ...
            @if (request()->anyFilled(['search', 'category_id', 'condition', 'status']))
                <a href="{{ route('units.show', $unit->id) }}" class="btn btn-outline btn-sm">Reset</a>
            @endif
        </form>

        @if ($assets->count() === 0)
            <div class="empty-state">Belum ada aset yang tercatat di unit ini.</div>
        @else
